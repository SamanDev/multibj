...
const WEB_URL = process.env.NODE_ENV === "production" ? `https://mbj.wheelofpersia.com/` : `https://mbj.wheelofpersia.com/`;
...
const getUser = async (token,clientId,theClient,ws) => {
  var data = await getUserService(token).then((userdata) => {
    ...
    var rooma = []
    for (let i = 8; i < app._router.stack.length; i++) {
      var game = games[WEB_URL+app._router.stack[i].route.path.replace("/", "")];
      rooma.push({room:WEB_URL+app._router.stack[i].route.path.replace("/", ""),players:game.spectators.length});
      //console.log(rooma)
    }
